"""Worker adapters for the Task 3 execution layer.
...
@dataclass
class WorkerResult:
    """Normalized result exchanged between adapters and the synthesizer."""
    ...
    def to_dict(self) -> dict[str, Any]:
        """Serialize without losing adapter/audit metadata."""
        return {
...
class Worker(Protocol):
    """Minimal adapter protocol used by WorkerPool."""

    worker_id: str

    def analyze(self, image_path: str, query: str, inspection_type: str, request_id: str) -> WorkerResult:
        ...


def _profile_for(image_path: str, profiles: dict[str, str]) -> str:
    """Resolve a deterministic profile from overrides or semantic file names."""
...
class MockWorker:
    """Deterministic adapter with evidence-shaped responses for local demos."""
    ...
    def _respond(self, profile: str, image_path: str, query: str, inspection_type: str) -> WorkerResult:
        ref = Path(image_path).name
        if self.worker_id == "Worker-A":
            if profile in ("shelf", "hybrid"):
                return WorkerResult(
                    self.worker_id, image_path,
                    "识别到零售货架，已按层记录商品和空货位；结果来自 Mock 视觉适配器。",
                    findings=[{"category": "inventory", "severity": "low", "description": "货架存在商品陈列，发现少量空货位，建议补货复核。", "image_ref": ref}],
                    compliance_items=[{"item": "货架陈列可见性", "status": "pass", "evidence": f"{ref} 中可见货架层和商品面。"}],
                    confidence=0.88,
                )
            if profile in ("safety", "conflict"):
                return WorkerResult(
                    ...
                    findings=[{"category": "safety_obstruction", "severity": "high", "description": "消防出口/通道附近出现疑似障碍物。", "image_ref": ref}],
                    compliance_items=[{"item": "消防通道无遮挡", "status": "fail", "evidence": f"{ref} 中出口前方可见障碍物或警示区域被占用。"}],
                    confidence=0.84,
                )
            return WorkerResult(self.worker_id, image_path, "未发现足够零售证据，无法可靠盘点。", confidence=0.42)
        ...
        if self.worker_id == "Worker-B":
            if profile == "open":
                return WorkerResult(
                    ...
                    findings=[{"category": "open_scene", "severity": "low", "description": "图片主要呈现开放域场景，不足以支持零售库存结论。", "image_ref": ref}],
                    confidence=0.91,
                )
            if profile == "conflict":
                return WorkerResult(
                    ...
                    findings=[{"category": "safety_obstruction", "severity": "med", "description": "可见警示区域，但障碍物是否侵入有效通道存在不确定性。", "image_ref": ref}],
                    compliance_items=[{"item": "消防通道无遮挡", "status": "pass", "evidence": f"{ref} 中未能确认有效通道被完全封堵。"}],
                    confidence=0.58,
                )
            return WorkerResult(self.worker_id, image_path, "通用视觉描述完成，未输出领域专项结论。", confidence=0.63)

        if self.worker_id == "Worker-C":
            return WorkerResult(
                self.worker_id, image_path,
                "文本汇总适配器只基于已有上下文生成报告，不重复执行视觉识别。",
                findings=[{"category": "report_context", "severity": "low", "description": "该结果可作为巡检报告汇总上下文，不能替代视觉证据。", "image_ref": ref}],
                confidence=0.77,
            )
...
class OpenAICompatibleOstrakonWorker:
    """Lazy HTTP adapter for a locally hosted OpenAI-compatible VLM."""
    ...
    def analyze(self, image_path: str, query: str, inspection_type: str, request_id: str) -> WorkerResult:
        ...
        try:
            ...
            return WorkerResult(
                self.worker_id, image_path, text,
            ...
            return WorkerResult(
                self.worker_id, image_path, "", confidence=0.0,
                latency_ms=(time.perf_counter() - started) * 1000,
...
class WorkerPool:
    """Dispatch A/B/C/D, with D executing A and B concurrently."""

    def __init__(self, mode: str = "mock", mock_profiles: dict[str, str] | None = None) -> None:
        if mode not in ("mock", "real"):
            raise ValueError("worker mode must be 'mock' or 'real'")
        self.mode = mode
        profiles = {str(Path(key).resolve()): value for key, value in (mock_profiles or {}).items()}
        if mode == "real":
            ...
            worker_a: Worker = OpenAICompatibleOstrakonWorker(endpoint, model, os.getenv("OSTRAKON_API_KEY", ""))
        else:
    ...
    def analyze(self, worker_id: str, image_path: str, query: str, inspection_type: str, request_id: str) -> WorkerResult:
        """Run a selected worker; D is a parallel A+B execution strategy."""
        if worker_id in self.workers:
            return self.workers[worker_id].analyze(image_path, query, inspection_type, request_id)
        ...
        started = time.perf_counter()
        with ThreadPoolExecutor(max_workers=2, thread_name_prefix="worker-d") as executor:
            future_a = executor.submit(self.workers["Worker-A"].analyze, image_path, query, inspection_type, request_id)
            future_b = executor.submit(self.workers["Worker-B"].analyze, image_path, query, inspection_type, request_id)
        ...
        return WorkerResult(
            ...
            metadata={"request_id": request_id, "source_results": [result_a.to_dict(), result_b.to_dict()]},
        )
