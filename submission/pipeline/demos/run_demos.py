...
DEMOS = [
    {
        ...
        "images": [
            OUT / "scenario_1_inventory" / "shelf_front.png",
            OUT / "scenario_1_inventory" / "shelf_oblique.png",
            OUT / "scenario_1_inventory" / "shelf_sparse.png",
        ],
        "profiles": {
            str((OUT / "scenario_1_inventory" / name).resolve()): "shelf"
            for name in ("shelf_front.png", "shelf_oblique.png", "shelf_sparse.png")
        },
        "expected_workers": {
            "shelf_front.png": "Worker-A",
            "shelf_oblique.png": "Worker-A",
            "shelf_sparse.png": "Worker-A",
        },
    },
    {
        ...
        "images": [
            OUT / "scenario_2_compliance" / "exit_obstructed.png",
            OUT / "scenario_2_compliance" / "fire_exit.png",
        ],
        "profiles": {
            str((OUT / "scenario_2_compliance" / name).resolve()): "safety"
            for name in ("exit_obstructed.png", "fire_exit.png")
        },
        "expected_workers": {
            "exit_obstructed.png": "Worker-D",
            "fire_exit.png": "Worker-D",
        },
    },
    {
        ...
        "images": [
            OUT / "scenario_3_comprehensive" / "open_domain.png",
            OUT / "scenario_3_comprehensive" / "shelf_hybrid.png",
            OUT / "scenario_3_comprehensive" / "conflict_exit.png",
        ],
        "profiles": {
            str(
                (OUT / "scenario_3_comprehensive" / "conflict_exit.png").resolve()
            ): "conflict"
        },
        "expected_workers": {
            "open_domain.png": "Worker-B",
            "shelf_hybrid.png": "Worker-D",
            "conflict_exit.png": "Worker-D",
        },
        "error_case": {
...
def main() -> None:
    ...
    for spec in DEMOS:
        ...
        report = pipeline.inspect(
            paths, spec["inspection_type"], spec["store_id"], request_id=spec["name"]
        )
        error_reason = (spec.get("error_case") or {}).get(
            "reason", "实际 Worker 与 Demo 预期不一致。"
        )
        ...
        (target / "routing_log.json").write_text(
            json.dumps(report["routing_log"], ensure_ascii=False, indent=2) + "\n",
            encoding="utf-8",
        )
        summary = {
            "title": spec["title"],
        ...
        index.append(
            {
                "name": spec["name"],
                "output": str((target / "output.json").relative_to(ROOT)),
