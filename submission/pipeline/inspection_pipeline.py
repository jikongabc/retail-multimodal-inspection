"""Task 3 retail inspection pipeline.

Python 3.10+; dependencies: numpy>=1.24, Pillow>=10.0.
"""
...
class InspectionPipeline:
    """Orchestrate routing, execution, temporary storage, and synthesis."""
    ...
    def _load_router() -> MultimodalRouter:
        ...
        if not data_path.exists():
            raise FileNotFoundError(f"Task 2 router weights/data not found under {router_dir}")
        if __package__ in (None, ""):
    ...
    @staticmethod
    def _validate_inputs(image_paths: Iterable[str | Path], inspection_type: str, store_id: str) -> list[str]:
        ...
        if inspection_type not in INSPECTION_TYPES:
            raise ValueError(f"inspection_type must be one of {sorted(INSPECTION_TYPES)}")
    ...
    def _result_is_mock(result: dict[str, Any]) -> bool:
        """Return whether a result or any nested D source came from a Mock."""
    ...
    def inspect(
        ...
        """Run one inspection request and return a validated report JSON object."""
        ...
        for index, image_path in enumerate(paths):
            ...
            worker_result = self.worker_pool.analyze(
                decision["worker"], image_path, decision["prompt_rewrite"], INSPECTION_TYPES[inspection_type], request_id,
            )
            worker_dict = worker_result.to_dict()
            results.append(worker_dict)
            key_prefix = f"inspection:{request_id}:image-{index + 1}:{decision['worker']}"
...
def main() -> None:
    """Small CLI used by the README and Demo runner."""
    ...
    parser.add_argument("--inspection-type", choices=sorted(INSPECTION_TYPES), default="综合评估")
    ...
    print(json.dumps(pipeline.inspect(args.images, args.inspection_type, args.store_id, args.request_id), ensure_ascii=False, indent=2))
