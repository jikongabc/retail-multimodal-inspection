...
本实现把 OpenFugu 的“轻量路由头 + 外部 Worker”扩展为图文路由：冻结图像/文本特征提取器，只训练一个四分类线性头；在线阶段先按请求特征选择专长 Worker，再用置信度、复杂度和不确定性触发 Worker-D 的 A+B 并行聚合。

这对应阶跃星辰公开的 `step-router-v1` 产品原则：根据请求特征在高能力引擎和高吞吐引擎之间自动分流；同时吸收其视觉模型“图像与文字联合输入、复杂请求增加推理深度、输出结构化结果”的工程用法。这里不声称复刻阶跃星辰的内部权重或算法，而是将公开的产品行为映射成可审计的本地路由策略。
...
* α：CLIP 图像 embedding + sentence embedding 拼接。跨模态语义最完整，但需要下载两个权重，首次加载和显存成本较高；本实现保留兼容接口，但不把外部权重下载作为实验前置条件。
...
最终选择 **γ-轻量混合变体** 作为本次默认实验后端：64 维图像颜色/纹理/边缘统计 + 96 维确定性哈希文本特征 + 16 维关键词/复杂度信号，并按 0.10/0.70/0.20 做图像/文本/任务信号模态归一化。它是 γ 的可复现扩展，不是 CLIP；代码通过 `MM_ROUTER_USE_CLIP=1` 保留 α 的生产替换接口。两种后端维度相同但特征分布不同，切换后必须重新训练 `router_weights.npy`，不能把 γ 权重冒充 CLIP 结果。选择 γ 的依据是：题目要求路由额外延迟小于 1 秒，当前没有把模型权重下载和 GPU 常驻作为路由前置条件，γ 能在离线环境真实复现并测量；α 是生产升级方向，本报告不虚报 α 精度。
...
`--mode train/evaluate` 会同时打印 clean test 的 raw/gated 两套结果和 hard-negative 结果；`route()` 返回 `raw_worker`、最终 `worker`、`gate_upgraded`、`gate_reasons` 和按意图生成的 `prompt_rewrite`，便于在 Pipeline 日志中审计升级原因。
