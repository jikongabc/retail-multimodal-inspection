...
路由器执行动作，环境返回金标或已验证奖励，ReAct 控制器据此观察、反思、生成训练样本、训练候选模型并执行发布门禁。

这里的“模型自己训练自己”不等于允许模型给自己生成标签。待训练路由器可以发现自身错误，但监督目标必须来自人工纠错、环境金标或独立验证器；模型自评只能进入人工审核队列。这个边界用于防止确认偏误不断放大。

## 2. 从 CogDoc 复用的反馈原则

反馈闭环遵循 CogDoc 的四项工程约束：

1. `FeedbackStore` 先保存原始反馈，差评另形成可进入评测的数据记录，而不是直接修改线上模型。
2. `feedback_understanding` 将反馈解析为类型、目标、置信度、动作和 `needs_review`；反馈决策对应结构化 Observation、Reflection 和 Action。
3. 检索调权记录支持 enable/disable；路由模型对应候选注册、门禁激活和父版本回滚。
4. 评测门禁同时检查绝对指标和相对基线，包括固定测试集 macro-F1、逐类 recall 和环境挑战收益。

CogDoc 使用 LangGraph 条件边，不是经典 ReAct 文本代理。此处采用确定性的 `Observe → Reflect → Act → Observe` 状态循环，不依赖外部 LLM。
...
Trace 只保存可审核的策略摘要和动作，不保存模型隐藏思维链。运行轨迹见 `react_trace.jsonl`。

## 5. 反馈质量与防污染

`feedback_store.py` 的约束：
...
通过门禁后保存 checkpoint、注册父版本、激活并热加载；失败候选不覆盖激活模型。`ModelRegistry.rollback()` 恢复父版本并记录原因和时间。
...
| `run_feedback_experiment.py` | 实验入口 |
