...
def main() -> None:
    ...
    parser.add_argument(
        "--feedback", default=str(Path(__file__).with_name("feedback.jsonl"))
    )
    parser.add_argument(
        "--registry", default=str(Path(__file__).with_name("model_registry.jsonl"))
    )
    parser.add_argument(
        "--checkpoint", default=str(Path(__file__).with_name("router_incremental.npy"))
    )
    parser.add_argument(
        "--output", default=str(Path(__file__).with_name("experiment_results.json"))
    )
    ...
    result = trainer.train_if_ready(
        "router-feedback-v1", args.checkpoint, minimum_feedback=5, generations=30
    )
